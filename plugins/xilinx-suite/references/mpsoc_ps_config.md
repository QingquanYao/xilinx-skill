...
## 适用器件：ZU15EG / ZU19EG / ZCU104（ZU7EV）

## 器件对比

| 项目 | ZU15EG | ZU19EG | ZU7EV（ZCU104）|
|------|--------|--------|----------------|
| 典型 Part | xczu15eg-ffvb1156-2-i | xczu19eg-ffvc1760-2-i | xczu7ev-ffvc1156-2-i |
| APU | 四核 Cortex-A53 | 四核 Cortex-A53 | 四核 Cortex-A53 |
| RPU | 双核 Cortex-R5F | 双核 Cortex-R5F | 双核 Cortex-R5F |
| GPU | Mali-400 MP2 | Mali-400 MP2 | Mali-400 MP2 |
| Video Codec | 无 | 无 | 有（VCU 硬核）|
| PL LUT | ~522K | ~1.14M | ~504K |
| PL DSP | ~1248 | ~2400 | ~1728 |
| PL BRAM | ~936 | ~2160 | ~312 |
| GT 收发器 | GTH | GTH | GTH |

**PS 配置方法完全相同**，只需换 part number。ZCU104 有官方 Board Support Package，支持一键预设。
...
## 完整 BD 脚本框架（ZU19EG 典型配置）

```tcl
# ============================================
# Block Design 创建脚本（ZU19EG 基础配置）
# ============================================
...
## ZCU104 开发板快速上手（Board Preset 方式）

ZCU104 有 Xilinx 官方 Board Support Package，可大幅简化 PS 配置：

```tcl
# 方法一：在创建工程时指定板卡
create_project my_proj ./my_proj -part xczu7ev-ffvc1156-2-i
...
# 应用 ZCU104 板卡预设（会自动填充 DDR4 时序、MIO 引脚分配等）
apply_bd_automation \
...
> 使用 Board Preset 后，DDR4、UART、Ethernet、USB、SD 等常用外设的 MIO 引脚会自动按 ZCU104 原理图配置，无需手动查引脚表。

**ZCU104 与自定义板的主要区别：**
- ZCU104 有官方 Board Files，支持 `apply_bd_automation` 一键预设
- 自定义板（如 ZU19EG 自制板）没有 Board Files，必须手动配置每个 PS 参数
- ZCU104 包含 VCU（Video Codec Unit）硬核，如需使用需额外配置 `v_proc_ss` 等 IP
